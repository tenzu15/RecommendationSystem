...
with open('pages/auth.yaml') as file:
...
if st.session_state["authentication_status"]:
    ...
    tab1 ,tab2, tab3, tab4 , tab5 = st.tabs(["Movies","Ideology","Original Dataset","Feature Enhancement", "DataSplit"])
    
    with tab1:
        DATA_URL = ("data/refined_dataset_final.csv")
    ...
    with tab2:
        ...
        st.write("DNN model is used to predict the ratings of the unseen movies. These movies are then ranked according to these ratings and the top 5 are recommended to the user.")
    ...
    with tab4:
        ...
        #df.drop("0")
        st.write(df.head())
        st.write(" We make changes to the dataset by combining the intial datasets based on Movie ID, followed by removing the duplicates")
        st.write(" We also make use of label encoders and normalization for encoding values accoeding to the DNN. Using DNN architecture helps us in avoding the pain of maually defining the features. Instead, the DNN automatically learns them based on the data given.")

    with tab5:

        st.write("Training & Test Set Split:")
        st.write("The training and test split for the dataset was as follows: 80% training data, 20% test data.")
        st.write("The user data was aggregated by userid and then split into 80% and 20% individually by user id.")
        st.write("Atlast, all the data was shuffled to keep it the process unbiased. Code Sample:")
        st.code(" uid= list(set(data['userId'])) \n X=pd.DataFrame() \n y=pd.DataFrame() \n for i in uid: \n  values=refined_dataset[refined_dataset['userId'] == i] \n  a,b=train_test_split(values, train_size=0.8) \n  X=X.append(a) \n  y=y.append(b) \n X=X.sample(frac=1) \n y=y.sample(frac=1) \n")
